// memory_write_gate.mjs — 记忆写入前置门（重建版，v2：自定位技能目录，消除 cwd 依赖）
// 原脚本随 2026-08-22 备份丢失未逐字还原；本版按 SKILL.md §6 接口重建。
// 用法: node scripts/memory_write_gate.mjs <目标文件> <临时文件>
//   目标文件: MEMORY.md | USER.md | notes/<file>.md（可绝对路径或相对技能目录）
// exit 0=允许（附核对：容量数字/占比、指针清单） 1=超容量（合并精简或下沉 notes/） 2=指针悬空/未注册（先建子文档或 INDEX 注册） 3=用法错误
...
const LIMITS = { 'MEMORY.md': 2200, 'USER.md': 1375, 'AGENT.md': 1375 };
const limit = isNotes ? 3000 : (LIMITS[base] ?? 2200);
