...
- `scripts/archive-lib.mjs` — (5.4 KB) 归档公共层：转录定位/解码/mark 读写/信号召回/pending 队列；env 隔离（插件与测试容器）
- `scripts/archive-mark.mjs` — (3.0 KB) 归档进度标记：upsert {lastRow,total,done,pending,lastTurnAt,fireAt}；--touch 重计时/--pending
- `scripts/archive-timer.mjs` — (6.3 KB) 方案 B 唤醒执行器：--due 到点触发→资格门控→落 pending 队列；--watch/--status/--pending-list/--dequeue
...
- `scripts/test.mjs` — (15.0 KB) 引擎回归测试：18 用例（门禁/召回/归档增量/方案B 状态机/env 隔离）
