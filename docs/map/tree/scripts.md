...
- `scripts/archive-timer.mjs` — (9.6 KB) 方案 B 唤醒执行器：--due 到点触发→资格门控→落 pending 队列；--watch/--status/--pending-list/--dequeue
...
- `scripts/test.mjs` — (21.2 KB) 引擎回归测试：18 用例（门禁/召回/归档增量/方案B 状态机/env 隔离）
