...
- `scripts/archive-lib.mjs` — (11.4 KB) 归档公共层：转录定位/解码/mark 读写/信号召回/pending 队列；env 隔离（插件与测试容器）
- `scripts/archive-mark.mjs` — (3.0 KB) 归档进度标记：upsert {lastRow,total,done,pending,lastTurnAt,fireAt}；--touch 重计时/--pending
- `scripts/archive-timer.mjs` — (11.7 KB) 方案 B 唤醒执行器：--due（机械消化碎片）+--drain 排空+活跃保护+数据重武装；--watch/--status/--pending-list/--dequeue
