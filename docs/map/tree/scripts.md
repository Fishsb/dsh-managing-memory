...
- `scripts/archive-lib.mjs` — (8.3 KB) 归档公共层：转录定位/解码/mark 读写/信号召回/pending 队列；env 隔离（插件与测试容器）
...
- `scripts/test.mjs` — (25.1 KB) 引擎回归测试：22 用例（门禁/召回/归档增量/方案B 状态机/env 隔离/机械消化/排空）
